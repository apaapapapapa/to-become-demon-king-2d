...
`EvolutionAppearanceProfile` は形態の色、表示倍率、演出種別だけを保持します。`PrototypeSlimeEvolutionPresenter` が `EvolutionApplied` を購読し、スライムのピクセルフレームを再着色して捕食系の角状エフェクトまたは魔術系の追従光を表示します。ロード済みSaveでは、初期化時に最後に取得した既知Nodeの外見を復元します。

外見プロファイルは静的Definitionであり、現在の形態IDや生成済みSpriteをSaveへ保存しません。本番用の専用Sprite、Prefab、Animator Controllerへの置き換えはPresentation内で行い、Evolution条件とSave形式へ影響させません。
...
両NodeのDefinition、条件・補正、選択UI、Prototype外見と演出は登録済みです。魔術系分岐が参照する火炎魔法ArtはRuntime未登録のため、現時点のPrototypeでは魔術系条件を満たせません。捕食系Skillと火炎魔法Artの正式な取得経路、本番用の専用アート、上位Nodeは後続タスクです。

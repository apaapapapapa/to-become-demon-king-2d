...
- Status: Foundation and Fire Magic Runtime Content Implemented
...
`ProgressionAcquisitionService` は `ProgressionGrantDefinition` を受け取り、Art習得とSkill解放を同じ取得境界から冪等に処理します。Prototypeでは見習い魔術師との一連の会話を終えると `grant.training.fire_magic` が送られ、`art.magic.fire` を習得します。
...
- `ProgressionGrantDefinition` / `ProgressionAcquisitionService` による取得元共通化
- 正式Art `art.magic.fire`、火炎弾 `ability.magic.fire_bolt`、NPC訓練取得経路
- Projectile命中による効果成立通知と火炎魔法の熟練加算

`IArtMasteryModifierSource` によるSkillとEvolutionの熟練ポイント補正まで接続済みです。火炎魔法は `K` / Gamepad Button Northから使用できます。複数Artの入力割当・選択UI、追加Ability、回復・バフ・デバフExecutorからの効果成立通知は今後接続します。

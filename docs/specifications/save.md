...
1. `PrototypeSaveSession` によるSave読込とVersion Migration
2. Player / Worldの基礎Runtime State生成
3. `PrototypeWorldBuilder` によるWorld / Player Runtime構築
...
Saveファイルが存在しない場合は新規状態で開始します。JSON破損、未対応Version、現在のPlayer Definitionと異なるCharacter IDなどで復元できない場合は、既存Saveの上書きを防ぐため新規状態で起動し、その起動中の保存を無効化します。
...
Gameplay Featureは保存先や保存タイミングを直接知りません。`PrototypeWorldBuilder` もSave DTOやQuest復元処理を知りません。

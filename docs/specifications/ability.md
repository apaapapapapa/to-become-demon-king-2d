...
- Skill: Abilityの獲得・強化などを行う成長要素
- Evolution: キャラクターの形態や成長経路を変える、不可逆または排他的な選択

Abilityは「何を実行するか」だけを扱い、Skillの取得状態やEvolution条件を知りません。
...
Executorは入力、Skill解放、Evolution条件、経験値、ドロップを処理しません。
...
Skill解放、Ability強化、Evolution条件判定、非同期Abilityの終了通知元は今後実装します。

...
Abilityは「何を実行するか」だけを扱い、Artの習得・熟練状態、Skillの取得状態、Evolution条件を知りません。能動行動は生得Ability、またはArtから解放されるAbilityとして表現します。
...
ArtからのAbility付与と近接ダメージの効果成立通知は実装済みです。受動Skill補正、Evolution条件判定、非同期Abilityの終了通知元、回復・バフ・デバフExecutorからの効果成立通知は今後実装します。詳細は [Art仕様](./art.md) を参照してください。

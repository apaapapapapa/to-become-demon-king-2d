...
このうち `DemonKing.Domain`、`CharacterDefinition`、`CharacterProgressionState`、Save DTO、`ISaveService`、型付きCombat境界、経験値計算、`RewardService` は導入済みです。SkillとEvolutionの実装はこの境界を利用して段階的に追加します。
...
経験値加算はUIやMonoBehaviourへ直接レベルアップ処理を書かず、`CharacterProgressionState.GainExperience` が `LevelUpResult` を返します。結果には加算前後のレベル、要求／実加算経験値、複数レベル上昇、最大レベル到達を含め、今後のUI、演出、スキルポイント付与はその結果を購読します。
...
敵や `PrototypeCombatDummy` からプレイヤーへ直接経験値を加算しません。訓練用ダミーは一意なDefeat IDを含む撃破結果と報酬Definition IDを公開し、`RewardService` が攻撃者ID、報酬ID、重複付与を検証して対象の `CharacterProgressionState` へ経験値を反映します。これにより、将来のクエスト、パーティー分配、経験値補正、実績を敵クラスから分離します。
...
- `ExperienceTable`、経験値加算、`LevelUpResult` とEditModeテスト
- `RewardService` と訓練用ダミー撃破から経験値加算までのPlayModeテスト

今後は次の順で成長機能を追加します。

1. 既存の通常攻撃をAbility実行基盤へ移す
2. アクティブスキル1個とパッシブスキル1個で基盤を検証する
3. 進化グラフ、解放状態、グラフ検証を追加する
4. `ISaveService` の具体実装とSave DTOの移行処理を追加する
5. 成長HUD、スキル画面、進化ツリー画面を追加する
...
- 厳密な単調増加を検証する累積 `ExperienceTable`
- 最大レベル時の余剰経験値保持／切り捨て方針
- 複数レベル上昇を含む `LevelUpResult`
- `ExperienceTableDefinition` と `RewardDefinition`
- 攻撃者ID、報酬ID、重複付与を検証する `RewardService`
- 訓練用ダミーの撃破通知からプレイヤー経験値加算までの接続
- Domain境界のEditModeテストと撃破報酬経路のPlayModeテスト

優先候補:

1. Ability実行基盤、スキル、進化を段階的に実装する
2. NPC会話、敵AI、クエストなど実ゲームコンテンツを増やす
3. 永続化対象が確定した時点で `ISaveService` の具体実装とデータ移行を追加する

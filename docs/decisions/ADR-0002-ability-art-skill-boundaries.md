...
- Skill補正は取得元を限定しないModifier Source契約として公開し、Ability、Combat、ArtがSkill取得状態を直接参照しない。
- 複数補正は固定値と割合をそれぞれ合算してから適用し、コンポーネント順への依存を避ける。
...
- 将来の装備、バフ、Evolution補正をSkillと同じ適用境界へ追加できる。
- Art進捗、Abilityとの対応、効果成立通知、Save Migrationの実装が必要になる。
- 複数Abilityを含むArtでは、ArtとAbilityのStable IDをそれぞれ管理する必要がある。
- `DamageTags.Art` を同じビット値で追加し、旧 `DamageTags.Skill` はObsoleteな互換Aliasとして移行する。

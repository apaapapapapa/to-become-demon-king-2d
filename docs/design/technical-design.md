...
Prototype専用のHUD、Dialogue Log、Quest Tracker / Notificationなど、現在Runtime Compositionする補助UIは既存方式を維持します。本格画面へ成長する場合は同じPrefabベース境界へ移行します。UI Toolkitへの全面移行は行いません。

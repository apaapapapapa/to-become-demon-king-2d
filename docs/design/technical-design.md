...
`EvolutionDefinition` はNode ID、対象Character、排他グループ、レベル・Skill・Artランク・前提Node条件、永続補正を保持します。`EvolutionProgressionService` が全条件を評価して成功時だけNode IDをRuntime Stateへ追加し、`EvolutionProgressionController` が選択済みNodeの補正を汎用Modifier Sourceへ公開します。

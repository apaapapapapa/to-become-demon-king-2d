...
`CharacterProgressionState` がLevel、累積経験値、Skill解放ID、Evolution Node解放IDを保持します。Art実装後は、キャラクター単位の `ArtProgressState` としてArt IDと累積Mastery Pointを追加します。
...
将来の `ArtDefinition` は `art.*` ID、熟練ランク閾値、複数のAbility解放定義を保持します。`ArtProgressState` はArt IDと累積Mastery Pointだけを保持し、ランクと解放AbilityをDefinitionから導出します。
...
詳細は [Ability仕様](../specifications/ability.md)、[Art仕様](../specifications/art.md)、[戦闘仕様](../specifications/combat.md)、[成長仕様](../specifications/progression.md) を参照してください。
...
Art実装後のSave DTOにはArt IDと累積Mastery Pointだけを追加します。現在ランク、解放Ability、装備Artは保存しません。既存のSave Version 1は空のArt進捗へMigrationします。
...
- Domain: ExperienceTable、CharacterProgressionState、将来のArtProgressState、Reward関連、Save Mapper等
